...
public class CalendarView extends View implements View.OnTouchListener {
    ...
    public List<SignInfo> getList_sign() {
        return list_sign;
    }

    public void setList_sign(List<SignInfo> list_sign) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String curYearAndMonthAndDay = getYearAndmonthNumber();
        ...
        String curYearAndMonthAndDay0 = "";
        //绘制日历时，查询签到记录，如果状态正常打钩，异常打叉
        for (int i = 0; i < 42; i++) {
            curYearAndMonthAndDay0 = curYearAndMonthAndDay + " " + date[i];
            int color = surface.textColor;
            String str_data = date[i] + "";
            if (isLastMonth(i)) {
                color = surface.borderColor;
            } else if (isNextMonth(i)) {
                color = surface.borderColor;
            } else if (todayIndex != 50 && i == todayIndex) {
                color = surface.todayNumberColor;
            } else {
                if (todayIndex == -1) {
                    if (curYear < todayYear || curYear == todayYear && curMonth < todayMonth) {
                ...
                if (i < todayIndex) {
                    ...
                    if (list_sign != null) {
                        for (SignInfo signInfo : list_sign) {
                            if (signInfo.getYearAndMounthAndDay().equals(curYearAndMonthAndDay0)) {
                                if (signInfo.getState() == Constant.SINGSTATE_NORMAL) {
                                    color = surface.textColor_right;
                                    str_data += "√";
                                } else {
                                    color = surface.textColor_woring;
                                    str_data += "×";
                                }
                                isRight = true;
                                break;
            ...
            drawCellText(canvas, i, str_data, color);
        }
        super.onDraw(canvas);
    }
    ...
    // 获得当前应该显示的年月
    public String getYearAndmonthNumber() {
        calendar.setTime(curDate);
        int year = calendar.get(Calendar.YEAR);
        int month = calendar.get(Calendar.MONTH);
        return "" + year + " " + (month + 1);
    ...
    private class Surface {
        ...
        private int textColor_right = Color.parseColor("#0A9C2A");
        private int textColor_woring = Color.parseColor("#E32F2F");
        ...
        public int todayNumberColor = Color.parseColor("#F1EC00"); //当天的颜色
        ...
        public void init() {
            ...
            monthHeight = 100;//(float) ((temp + temp * 0.3f) * 0.6);
